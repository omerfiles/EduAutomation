...
public class HtmlLogLayout extends HTMLLayout {
	...
	public String getHeader() {
		...
		sbuf.append("body, table {font-family: arial,sans-serif; font-size: large;}"
				+ Layout.LINE_SEP);
...
//		sbuf.append(Transform.escapeTags(event.getRenderedMessage()));
		...
		if (event.getRenderedMessage().contains("http")) {
			int start = event.getRenderedMessage().indexOf("http");
			int end = event.getRenderedMessage().indexOf(" ", start);
			if(end==-1){
				end=event.getRenderedMessage().length();
			}
			String link = event.getRenderedMessage().substring(start, end);
		
			String newMessage = event.getRenderedMessage().replace(link, "");
			sbuf.append(newMessage);
			sbuf.append(" <a href='"+link+"' target='_blank'>link</a>");
		}
		else{
